...
void HardAi::execute() {
    listCountries = state->getListCountires();
    playerlistcountries = player->getListCountry();
    bonus = player->continentBonusTroop();

    //initialiser la liste
    for(int i = 0; i < listCountries.size(); i++){
        if(i < 9) ameriqueN.push_back(listCountries[i]->getNumberTroop());
        else if(i < 13) ameriqueS.push_back(listCountries[i]->getNumberTroop());
        else if(i < 19) afrique.push_back(listCountries[i]->getNumberTroop());
        else if(i < 26) europe.push_back(listCountries[i]->getNumberTroop());
        else if(i < 38) asie.push_back(listCountries[i]->getNumberTroop());
        else ocenanie.push_back(listCountries[i]->getNumberTroop());
    }
